...
/**
 * A manager for selecting punishments with specific details from the database.
 * This, along with {@link SelectionOrder}, provide an API for efficient
 * punishment queries. <br>
 * <br>
...
public interface PunishmentSelector {
	...
	/**
	 * Begins creating a selection order by returning a
	 * {@link SelectionOrderBuilder}
	 * 
	 ...
	 * {@link #getActivePunishmentByIdAndType(int, PunishmentType)} should be
	 * preferred.
	 * 
	 ...
	 *         one
	 */
	ReactionStage<Optional<Punishment>> getActivePunishmentById(int id);

	/**
	 * Gets an active punishment matching a specific ID and type, if one exists with
	 * matching type and ID. <br>
	 * <br>
	 * Unlike {@link #getActivePunishmentById(int)}, this method may be more
	 ...
	 *         there is one
	 */
	ReactionStage<Optional<Punishment>> getActivePunishmentByIdAndType(int id, PunishmentType type);
	 ...
	 *         is one
	 */
	ReactionStage<Optional<Punishment>> getHistoricalPunishmentById(int id);
	 ...
	 * connected player. <br>
	 * <br>
	 * Applicability is synonymous with enforceability. For example, if a ban would
	 * prevent a player from joining the server, the ban is said to be applicable to
	 * the player's UUID and IP. It may be, the player's IP is banned, the player's
	 * UUID is banned, or the player has played on a banned IP and that IP is banned
	 * while strict address enforcement is enabled. That is, this method takes into
	 * account configuration options related to enforcement.
	 * 
	 * @param uuid    the player's UUID
	 * @param address the player's current address
	 * @param type    the punishment type
	 * @return a future which yields the first applicable punishment for the id and
	 *         address, if there is one
	 */
	ReactionStage<Optional<Punishment>> getApplicablePunishment(UUID uuid, NetworkAddress address, PunishmentType type);
	 ...
	 * <br>
	 * If the mute is cached, a completed future is returned. Else, an in-progress
	 * future is returned.
	 * 
	 * @param uuid    the player's uuid
	 * @param address the player's current address
	 * @return a future yielding either a cached value or the queried mute, if there
	 *         is one
	 */
