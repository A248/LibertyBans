...
Logins and mute checks are the most frequently issued queries, so this is the most important section.
...
explain select "victim_type", "victim_uuid", "victim_address", "operator", "reason", "start", "end", "track", "id"
from "libertybans_applicable_bans"
where (("end" = 0 or "end" > 1621440000)
  and ("libertybans_applicable_bans"."uuid" = X'03a27a2a7f5c4bcbb369422de19f741f'
    or "libertybans_applicable_bans"."address" in (
      X'1EF97F89',
      X'C07D66F4',
      X'88D97498C221FD69BC893E0E6F37EC1A',
      X'A689353E',
...
from (
  ...
  where "libertybans_applicable_bans"."uuid" = X'2c427ad2c6544a6ba59027e5923ef1d0'
  union all
  select "libertybans_applicable_bans".* from "libertybans_applicable_bans"
  where ("libertybans_applicable_bans"."victim_type" <> 0 and "libertybans_applicable_bans"."address" in (
    select "libertybans_addresses"."address" from "libertybans_addresses"
    where "libertybans_addresses"."uuid" = X'2c427ad2c6544a6ba59027e5923ef1d0'
    union all
    select X'6689353E442B5E15AC6200EC96A8CB67' from dual
  ))
) as "sq_union"
where ("end" = 0 or "end" > 1621440000)
order by case "end" when 0 then 9223372036854775807 else "end" end desc limit 1;
...
from (
  ...
  where ("libertybans_applicable_bans"."victim_type" <> 0 and "libertybans_applicable_bans"."address" in (
    X'1EF97F89',
    X'C07D66F4',
    X'6689353E442B5E15AC6200EC96A8CB67'
  ))
) as "sq_union"
where ("end" = 0 or "end" > 1621440000)
order by case "end" when 0 then 9223372036854775807 else "end" end desc limit 1;
...
from (
  select
    max("victim_type") as "inner_victim_type", max("victim_uuid") as "inner_victim_uuid",
    max("victim_address") as "inner_victim_address", max("libertybans_applicable_history"."type") as "inner_type",
    max("libertybans_applicable_history"."operator") as "inner_operator", max("libertybans_applicable_history"."reason") as "inner_reason",
    max("libertybans_applicable_history"."scope_type") as "inner_scope_type", max("libertybans_applicable_history"."scope") as "inner_scope",
    max("libertybans_applicable_history"."start") as "inner_start", max("libertybans_applicable_history"."end") as "inner_end",
    max("libertybans_applicable_history"."track") as "inner_track", "id"
