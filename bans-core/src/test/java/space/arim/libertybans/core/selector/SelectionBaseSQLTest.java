...
public class SelectionBaseSQLTest {
	...
	private String expectedSql(AddressStrictness strictness) {
		return switch (strictness) {
			case LENIENT -> """
select "libertybans_simple_bans"."id", "libertybans_simple_bans"."victim_type", \
...
"libertybans_simple_bans"."start", "libertybans_simple_bans"."end" from "libertybans_simple_bans" where \
...
"libertybans_simple_bans"."victim_address" = cast(? as varbinary(16))))) and ("libertybans_simple_bans"."end" = 0 or \
"libertybans_simple_bans"."end" > cast(? as bigint))) \
...
when 0 then 9223372036854775807 else "libertybans_simple_bans"."end" end desc limit 1""";
			case NORMAL -> """
select "libertybans_applicable_bans"."id", "libertybans_applicable_bans"."victim_type", \
...
"libertybans_applicable_bans"."end" from "libertybans_applicable_bans" \
where ("libertybans_applicable_bans"."uuid" = cast(? as uuid) and ("libertybans_applicable_bans"."end" = 0 \
or "libertybans_applicable_bans"."end" > cast(? as bigint))) \
...
where (("libertybans_strict_links"."uuid1" = cast(? as uuid) or ("libertybans_strict_links"."uuid2" = cast(? as uuid) \
and "libertybans_applicable_bans"."victim_type" <> 0)) and ("libertybans_applicable_bans"."end" = 0 or \
"libertybans_applicable_bans"."end" > cast(? as bigint))) \
order by case "libertybans_applicable_bans"."end" \
when 0 then 9223372036854775807 else "libertybans_applicable_bans"."end" end desc limit 1""";
			case STRICT -> """
select "libertybans_applicable_bans"."id", "libertybans_applicable_bans"."victim_type", \
...
"libertybans_applicable_bans"."end" from "libertybans_applicable_bans" \
join "libertybans_strict_links" on "libertybans_applicable_bans"."uuid" = "libertybans_strict_links"."uuid1" \
where ("libertybans_strict_links"."uuid2" = cast(? as uuid) and \
